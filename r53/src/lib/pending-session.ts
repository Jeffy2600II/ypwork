...
// ═══════════════════════════════════════════════════════════════
// จัดการสถานะ "pending" ของผู้ใช้ที่ลงทะเบียนแล้ว แต่ยังไม่ถูกอนุมัติ
//
...
export interface PendingSession {
  ...
  /** ชื่อ-นามสกุลที่กรอกตอนลงทะเบียน */
  full_name: string;
  /** ประเภทบัญชี */
  account_type: RegisterAccountType;
  /** เวลาที่ลงทะเบียน (ISO string) */
  submitted_at: string;
  /**
...
/**
 * ★ v3.7.6: ล้าง auth-related localStorage ทั้งหมด — แก้ปัญหา logout แล้ว login ไม่ได้
 *
 * ปัญหา: หลัง logout ระบบล้างเฉพาะ Supabase cookies แต่ไม่ได้ล้าง localStorage
 * → yp_rejected_accounts ยังคงอยู่ → เมื่อ login ใหม่ ระบบตรวจพบว่า "ถูกปฏิเสธ"
 *   ทั้งที่จริงๆ บัญชีถูกอนุมัติแล้ว → แสดงข้อความ "ไม่พบบัญชี"
 *
 * วิธีแก้: ล้าง localStorage ทั้งหมดที่เกี่ยวข้องกับ auth ฝั่ง client
 * เรียกก่อน hard navigate ไป /login
 */
export function clearAllAuthStorage(): void {
  if (typeof window === 'undefined') return;
  try {
    // ล้าง pending session
    window.localStorage.removeItem(PENDING_KEY);
    // ล้าง rejected accounts (สำคัญมาก — ทำให้ login ไม่ได้)
    window.localStorage.removeItem(REJECTED_KEY);
    // ล้าง storage อื่นๆ ที่อาจค้างอยู่
    window.localStorage.removeItem('yp_csrf_token');
    window.localStorage.removeItem('yp_session_user');
    window.localStorage.removeItem('yp_tutorial_seen');

    // ส่ง event ให้ component อื่น ๆ รู้ว่า state เปลี่ยน
    window.dispatchEvent(new CustomEvent('yp-pending-session-change'));
    window.dispatchEvent(new CustomEvent('yp-rejected-accounts-change'));

    console.log('[clearAllAuthStorage] cleared all auth localStorage');
  } catch {
    // ignore
  }
}
...
/** ล้างรายการปฏิเสธ (สำหรับกรณี "ลงทะเบียนใหม่อีกครั้ง") */
