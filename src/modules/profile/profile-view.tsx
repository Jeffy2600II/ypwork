...
export function ProfileView({ user: initialUser, department, stats }: ProfileViewProps) {
  ...
  const handleLogout = async () => {
    ...
    // ★ v3.7.6: แก้ปัญหา logout แล้ว login ไม่ได้ — ล้าง localStorage ทั้งหมด
    //
    //   ปัญหา v3.7.5: หลัง logout ระบบล้างเฉพาะ Supabase cookies แต่ไม่ได้ล้าง localStorage
    ...
    //
    //   วิธีแก้ v3.7.6:
    //   1. ล้าง localStorage ทั้งหมดก่อน (clearAllAuthStorage)
    //   2. เรียก /api/auth/logout (server-side) เพื่อล้าง cookies
    //   3. Hard navigate ไป /login?logged_out=1
    try {
      ...
      // ถึงแม้จะ error ก็ navigate ไป /login อยู่ดี — middleware จะจัดการ
    } finally {
      // ★ Hard navigation ด้วย window.location.replace
