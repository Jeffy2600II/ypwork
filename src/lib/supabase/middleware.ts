 ...
 *       สำหรับผู้ใช้ที่ส่งคำขอสมัครแล้วแต่ยังไม่อนุมัติ — ดูสถานะ realtime
 *
...
export async function updateSession(request: NextRequest) {
  ...
  // v1.9: /pending-status เป็น public route (เหมือน /login, /register)
  //       ผู้ใช้ที่ส่งคำขอสมัครแล้วแต่ยังไม่อนุมัติใช้ดูสถานะแบบ realtime
  ...
  // (user ที่ approved แล้วไม่ควรเห็นหน้า pending-status)
  if (user && isAuthRoute) {
