...
export function Window({
  ...
  // ★ r48 (Bug #1+#2 fix): เปลี่ยน dep จาก `open` เป็น `mounted`
  //   เหตุผล: ต้องการให้ window ยัง "registered" อยู่ใน stack ไปจนถึง end ของ
  //   close animation (mounted=false) — ไม่ใช่แค่ตอน open=false
  //   ไม่งั้น body.yp-window-open ถูกลบก่อน close animation เสร็จ →
  //   useScrollDirection คิดว่า window ปิดแล้ว → อ่าน window.scrollY=0 →
  //   ตั้ง fabHidden=false → FAB flash (Bug #1)
  React.useEffect(() => {
    if (!mounted) return;
  ...
  // ★ r48 (Bug #2 fix): แยก scroll lock ออกจาก close animation
  //   ปัญหาเดิม: effect deps มี isClosing → พอ setIsClosing(true) cleanup รันทันที
  //   → unlockScroll ทำ body position กลับเป็น static + window.scrollTo ทันที
  //   → page ใต้ sheet กระโดดกลับตำแหน่งเดิม (วาร์ป) ขณะที่ sheet ยังเลื่อนลงอยู่
  //
  //   วิธีแก้: ไม่ใส่ isClosing/isOpen ใน deps — lock ตอน mounted, unlock ตอน unmount
  //   ทำให้ page ใต้ sheet ค้าง scroll-locked จนกว่า sheet จะ unmount (หลัง anim)
  //   ส่งผล: sheet เลื่อนลงจนสุด → unmount → unlockScroll → page ปรากฏที่เดิมนิ่งๆ
  React.useEffect(() => {
    if (!mounted) return;
